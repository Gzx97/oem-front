  ...
  render() {
    ...
    return (
      <div id="record">
        ...
        ></textarea>
        <AccountHeader />
        <div className="it_title">盲盒购买记录</div>
        {/* <div className="it_title">产品买记录</div> */}
                ...
                <li key={item.id}>
                  ...
                  <p>
                    <span
                      onClick={() => {
                        if (item.payStatus == "0") {
                          // window.location.href=`http://47.52.21.155:8200/api/commodity/chinapaysubmit?order_id=${item.id}`;  
                        }
                      }}
                      className={this.getStatusLabel(item.payStatus).className}
                    >
                      {this.getStatusLabel(item.payStatus).label}
                    </span>
                  </p>
                  {/* <p>状态：{this.getStatusLabel(item.payStatus).label}</p> */}
                  <div className="time-address">盲盒名称：{item.boxName}</div>
                </li>
              );
            })}
            {/* <li>
